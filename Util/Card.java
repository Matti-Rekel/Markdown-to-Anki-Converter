...
public class Card{
  ...
  public static Card formatFields(Card card, Path inputPath, Path ankiMediaPath){
  ...
  public static String formatCard(Card card, char separator){
    String note = "";
  ...
  private static String imageHandler (String field, Path fileSource, Path ankiMediaPath){
    ...
    // Paterns for Images and Image source in Field
    ...
    while (imageMatcher.find()){ // While a match is found
      Matcher imageSrcMatcher = imageSrcPattern.matcher(imageMatcher.group(1)); // look for the source in the Image-Element
      imageSrcMatcher.find();
      Path source = Paths.get(imageSrcMatcher.group(1));
      try {
        //checking whether the source Path is absolute or relative if relative converting it to absolute
        // Also copying the images into Ankis Media Folder
    ...
    // replacing the src in the original HTML by ignoring the parts of the Path except for File name and extension
