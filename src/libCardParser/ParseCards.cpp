...
auto split_text(std::string text, std::map<std::string, fields> keywords) -> Card
{
    ...
    std::map<fields, std::string> extraFields = find_extra_fields(text, index, keywords);

    for (fields a : {fields::Question, fields::Prerequisites, fields::Hint, fields::Answer, fields::Add_Information,
                     fields::Source})
    {
...
auto find_extra_fields(std::string text, size_t& index, std::map<std::string, fields> keywords)
    ...
    while (index < text.length())
    {
        if ((index + 3 < text.length()) &&
            (text.at(index - 1) == '\n' && text.at(index) == '#' && text.at(index + 1) == '#' &&
             text.at(index + 2) == '#' && text.at(index + 3) == ' '))
        {
            i.push_back(index);
        }
        index++;
